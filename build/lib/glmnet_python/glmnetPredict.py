...
def glmnetPredict(fit,\
    ...
    if fit['class'] in ['elnet', 'fishnet', 'lognet']:
        ...
        if np.size(s) > 0:
            ...
            nbeta = nbeta[:, lamlist['left']]*np.tile(np.transpose(lamlist['frac']), [nbeta.shape[0], 1]) \
            + nbeta[:, lamlist['right']]*( 1 - np.tile(np.transpose(lamlist['frac']), [nbeta.shape[0], 1]))
        ...
        result = np.dot(np.column_stack( (np.ones([newx.shape[0], 1]) , newx) ) , nbeta)
    ...
    if fit['class'] == 'mrelnet' or fit['class'] == 'multnet':
        ...
        if len(s) > 0:
            ...
            for i in range(nclass):
                ...
                kbeta = kbeta[:, lamlist['left']]*np.tile(np.transpose(lamlist['frac']), [kbeta.shape[0], 1]) \
                        + kbeta[:, lamlist['right']]*( 1 - np.tile(np.transpose(lamlist['frac']), [kbeta.shape[0], 1]))
                nbeta[i] = kbeta
        else:
            for i in range(nclass):
        ...
        for i in range(nclass):
            ...
            dp[i, :, :] = dp[i, :, :] + np.reshape(np.transpose(fitk), [1, nlambda, npred])
