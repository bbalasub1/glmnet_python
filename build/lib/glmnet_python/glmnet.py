...
INPUT ARGUMENTS:
---------------
  x        Input np 2D array of nobs x nvars (required). Each row is an 
           observation vector. Can be in sparse matrix format. Must be in 
           np csc_matrix format
           
  y        Response variable (np 2D array of size nobs x 1, nobs x nc, etc). (required) 
  ...
  options  optional parameters that can be set and altered by glmnetSet()
           Default values for some often used parameters:
             ...
             weights = all ones np vector (observation weights)
...
--------
      ...
      glmnetPrint(fit)
      glmnetPredict(fit, np.empty([0]), np.array([0.01]), 'coef') # extract coefficients at a single value of lambdau
      glmnetPredict(fit, x[0:10,:], np.array([0.01, 0.005])) # make predictions
...
def glmnet(*, x, y, family='gaussian', **options):
    ...
    # check inputs: make sure x and y are np, float64 arrays
    ...
    if not( isinstance(x, scipy.sparse.csc.csc_matrix) ):
        if not( isinstance(x, np.ndarray) and x.dtype == 'float64'):
            raise ValueError('x input must be a np float64 ndarray')
    else:
        if not (x.dtype == 'float64'):
            raise ValueError('x input must be a float64 array')
            
    if not( isinstance(y, np.ndarray) and y.dtype == 'float64'):
            raise ValueError('y input must be a np float64 ndarray')
    ...
    # check if weights are np nd array
    if not( isinstance(weights, np.ndarray) and weights.dtype == 'float64'):
        raise ValueError('weights input must be a np float64 ndarray')
    ...
    if not (len(exclude) == 0):
        ...
        if np.any(exclude < 0) or np.any(exclude >= nvars):
            ...
            jd = np.append(len(exclude), exclude + 1) # indices are 1-based in fortran
    else:
    ...
    if scipy.sparse.issparse(x):
        ...
        irs = tx.indices + 1
        pcs = tx.indptr + 1
        irs = np.reshape(irs, [len(irs),])
        pcs = np.reshape(pcs, [len(pcs),])        
    else:
        irs = np.empty([0])
        pcs = np.empty([0])
